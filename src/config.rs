...
pub const PUBLIC_UI_HOST: &str = "gopher.localhost";
...
impl Config {
    ...
    pub fn resolve_ui_bind(cli_bind: Option<SocketAddr>, config: &Self) -> Result<SocketAddr> {
        let env_bind = std::env::var("GOPHER_BIND").ok();
        ...
        let addr = if let Some(addr) = cli_bind {
            ...
            if !trimmed.is_empty() {
                SocketAddr::from_str(trimmed)
                    .with_context(|| format!("invalid GOPHER_BIND value: {raw}"))?
            } else {
...
/// Reject non-loopback binds. Gopher has no auth and must not be LAN/WAN-exposed.
pub fn require_loopback_bind(addr: SocketAddr) -> Result<SocketAddr> {
    ...
    eprintln!(
        "WARNING: Gopher is loopback-only. It has no authentication and can read/write local data."
    );
    eprintln!("WARNING: Use 127.0.0.1 or ::1 (config ui.host, --bind, or GOPHER_BIND).");
    eprintln!();
...
mod tests {
    ...
    fn public_ui_url_uses_friendly_localhost_host() {
        ...
        assert_eq!(public_ui_url(bind), "http://gopher.localhost:3930");
        let custom: SocketAddr = "127.0.0.1:4000".parse().unwrap();
        assert_eq!(public_ui_url(custom), "http://gopher.localhost:4000");
    }
